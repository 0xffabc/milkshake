...
object LiquidWalk : Module(
  ...
  private fun getMostTopFluidPos(entity: Entity): BlockPos? {
    (0..3).forEach {
      val blockPos = if (entity.isSubmergedInWater()) entity.getBlockPos().up(it) else entity.getBlockPos().down(it)
      ...
      if (!entity.isSubmergedInWater()) return@getMostTopFluidPos waterPos
    }
